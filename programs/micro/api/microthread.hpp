...
#include <cstdlib>
#include <memory>
#define THREAD_SYSCALLS_BASE 590
...
{
struct Thread;
extern "C" long sys_microthread_exit(long);
extern "C" long sys_microthread_yield();
extern "C" long sys_microthread_yield_to(int);
extern "C" long sys_microthread_block(int);
extern "C" long sys_microthread_unblock(int);
extern "C" long sys_microthread_wakeup_one_blocked(int);
extern "C" long sys_microthread_create(void* sp, void(*)(Thread*), void* tls, long flags, void* stack_base, long stack_size);
extern "C" long sys_microthread_direct(void(*)(), void(*)());
...
inline auto create(const T& func, Args&&... args)
{
	...
	extern void trampoline(struct Thread*);
	/* stack, func, tls, flags = CHILD_CLEARTID, stack_base, stack_size */
	void* sp   = (void*) stack_top;
	void* tls  = (void*) thread;
	(void)sys_microthread_create(sp, &trampoline, tls, 0x200000,
		stack_bot, Thread::STACK_SIZE);
...
inline int oneshot(const T& func, Args&&... args)
{
	...
	char* stack_bot = (char*) malloc(Thread::STACK_SIZE);
	if (stack_bot == nullptr) return -12; /* ENOMEM */
	...
	void* sp   = (void*) stack_top;
	void* tls  = (void*) thread;
	return sys_microthread_create(sp, &trampoline, tls, 0, stack_bot, Thread::STACK_SIZE);
...
inline long direct(direct_funcptr starter)
{
	...
	return a0_out;*/
	return sys_microthread_direct(starter, threadcall_destructor);
}
