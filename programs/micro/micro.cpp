...
#define NATIVE_SYSCALLS_BASE 570 /* libc starts at 570 */
...
extern "C" void *__wrap_memcpy(void *vdest, const void *vsrc, size_t size) {
	...
	asm volatile("ecall"
				 : "=m"(*(char(*)[16384])a0), "+r"(a0)
				 : "r"(a1), "m"(*(const char(*)[16384])a1),
				 "r"(a2), "r"(syscall_id));
	return vdest;
}
extern "C" void *__wrap_memmove(void *vdest, const void *vsrc, size_t size) {
	...
	asm volatile("ecall"
				 : "=m"(*(char(*)[16384])a0), "=m"(*(char(*)[16384])a1)
				 : "r"(a0), "r"(a1), "r"(a2), "r"(syscall_id));
	return vdest;
}
...
CREATE_SYSCALL(sys_assert_fail, ECALL_ASSERT_FAIL);
CREATE_SYSCALL(sys_write,   ECALL_WRITE);
CREATE_SYSCALL(sys_measure, ECALL_MEASURE);
...
extern "C" __attribute__((used, retain, noreturn))
void fast_exit(int code)
{
	register long a0 asm("a0") = code;
	asm (".insn i SYSTEM, 0, x0, x0, 0x7ff" : : "r"(a0));
