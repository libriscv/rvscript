...
struct SystemTimer
{
    ...
    SystemTimer(duration_t p, int rep, handler_t cb)
        : period(p), callback(std::move(cb)), repeats(rep), deferred_destruct(false)
    {}
    ...
    bool is_alive() const noexcept { return deferred_destruct == false; }
    bool is_oneshot() const noexcept { return repeats == 0; }
    void reset()
    {
        callback = nullptr;
...
public:
    ...
    void set_repeats(int id, int repeats) { timers.at(id).repeats = repeats; }
    int get_repeats(int id) { return timers.at(id).repeats; }
