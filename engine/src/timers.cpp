    ...
    while (!this->scheduled.empty())
    {
        ...
        else
        {
            ...
            if (ts_now >= when)
            {
                ...
                // reduce repeat count
                if (timers[id].repeats > 0) timers[id].repeats--;
                ...
                {
                    timer.reset();
                    if (timer.deferred_destruct) this->dead_timers--;
                    this->free_timers.push_back(id);
                }
                else
                {
                    ...
                    this->scheduled.emplace(std::piecewise_construct,
...
void Timers::sched_timer(duration_t when, int id)
{
    ...
    this->scheduled.emplace(std::piecewise_construct, std::forward_as_tuple(now() + when),
                            std::forward_as_tuple(id));
}
