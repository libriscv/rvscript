...
void Script::gdb_remote_debugging(
	std::string message, bool one_up, uint16_t port)
{
	...
	if (0 == fork())
	{
		...
		const char* argv[]
			= {getenv_with_default("GDB", "/usr/bin/gdb-multiarch"), "-x",
			   scrname, nullptr};
	...
	Script::set_dynamic_calls(
		{{"Debug::breakpoint", "void sys_breakpoint (uint16_t, const char*)",
		  [](Script& script)
		  {
			  ...
			  if (script.is_debug())
			  {
				  // We have to pre-emptively skip over the breakpoint
				  // instruction
				  machine.cpu.jump(machine.cpu.pc() + 4);
				  // Wait for someone to connect:
				  auto [port, info] = machine.sysargs<uint16_t, std::string>();
				  auto message		= strf::to_string(
						   "Breakpoint in ", script.name(), ":0x",
						   strf::hex(machine.cpu.pc()), ".\t", info, "\t\t");
			  ...
			  {
				  strf::to(stderr)(
					  script.name(), ": Skipped over breakpoint at ",
					  (void*)machine.cpu.pc(), "\n");
			  }
		  }},
