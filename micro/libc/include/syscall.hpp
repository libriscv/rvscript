...
#define NATIVE_SYSCALLS_BASE   80
...
#define SYSCALL_MEMCMP    (NATIVE_SYSCALLS_BASE+7)
#define SYSCALL_BACKTRACE (NATIVE_SYSCALLS_BASE+8)

#define THREAD_SYSCALLS_BASE   90

/** General purpose function for system calls **/
/** Note: floats will be passed as 64-bit doubles **/
...
inline long
fsyscall(long n, float farg0, float farg1)
{
	register long   a0 asm("a0");
	register float fa0 asm("fa0") = farg0;
...
inline long
fsyscall(long n, float farg0, float farg1, float farg2)
{
	register long   a0 asm("a0");
	register float fa0 asm("fa0") = farg0;
	register float fa1 asm("fa1") = farg1;
	...
	asm volatile ("scall"
	 	: "=r"(a0) : "r"(fa0), "r"(fa1), "r"(fa2), "r"(syscall_id));
